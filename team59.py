...
class Team59():
	...
	def big_board_heuristic_func(self,blockheur):
		...
		for i in range(3):
			for j in range(3):
				if blockheur[i][j]>0:
					ret+=(0.2*self.draw_weight[i][j]+0.8*self.pos_weight[i][j])*blockheur[i][j]
		return ret
	...
	def minimax(self,board,flag,depth,max_depth,alpha,beta,old_move):
		...
		if is_max:
			...
			for i in range(len(valid_cells)):
				...
				if beta<=alpha:
					break
				if time.time()-self.curr_time >= 0.99:
	...
	def move(self,board,old_move,flag):	
		...
		depth = 2 #depth for iterative deepening
		valid_cells=board.find_valid_move_cells(old_move)
